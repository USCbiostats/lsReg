...
base <- glm(ypois ~ x1 + x2, data = dat, family = poisson)
xr_z5 <- as.matrix(dat[, "z5", drop = FALSE])
xr_z1 <- as.matrix(dat[, "z1", drop = FALSE])

full_z5 <- glm(ypois ~ x1 + x2 + z5, data = dat, family = poisson)
full_z1 <- glm(ypois ~ x1 + x2 + z1, data = dat, family = poisson)

test_that("Poisson Wald test statistic matches glm", {
  mem <- lsReg(base, colstoadd = 1, testtype = "wald")
  addcovar(mem, xr_z5)
  expect_equal(mem$testvalue[1, 1],
               coef(summary(full_z5))["z5", "z value"], tolerance = 1e-5)
  addcovar(mem, xr_z1)
  expect_equal(mem$testvalue[1, 1],
               coef(summary(full_z1))["z1", "z value"], tolerance = 1e-5)
})

test_that("Poisson Wald parameter estimate matches glm", {
  mem <- lsReg(base, colstoadd = 1, testtype = "wald")
  addcovar(mem, xr_z5)
...
test_that("Poisson LRT statistic matches glm log-likelihood ratio", {
...
test_that("Poisson score test statistic matches statmod::glm.scoretest", {
...
test_that("Poisson robust score test statistic is finite", {
...
test_that("Poisson robust Wald test statistic is finite", {
...
test_that("Poisson robust statistics are similar to standard under correct model", {
  ...
  expect_equal(mem_wald$testvalue[1,1],  mem_rwald$testvalue[1,1],  tolerance = 1.0)
