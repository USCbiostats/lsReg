...
2. **Test** — call `runtest()` once per candidate covariate, retrieving the
...
Loop over `z1` through `z10`, calling `runtest()` for each. After each call the
...
The variables `z5` and `z9` show large Wald statistics and small p-values,
consistent with the data-generating model. The remaining variables are null and
show statistics close to zero.